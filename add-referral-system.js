...
async function addReferralSystem() {
  try {
    ...
    console.log('📝 Adding referral fields to vendors table...');
    await client.query(`
      ALTER TABLE vendors
      ADD COLUMN IF NOT EXISTS referred_by UUID REFERENCES users(id) ON DELETE SET NULL,
    ...
    console.log('📝 Adding referral system settings...');
    await client.query(`
    ...
    console.log('  - Added 4 columns to vendors table');
    console.log('  - Created referral_transactions table');
    console.log('  - Created 7 indexes');
    console.log('  - Added 4 system settings');
...
if (require.main === module) {
  addReferralSystem()
    .then(() => {
      console.log('\n🎉 All done! You can now restart your backend service.');
      process.exit(0);
    })
    .catch((error) => {
      console.error('\n💥 Migration failed!');
